...
% % data load
% data  = load("Bellhop_jitter_noreverb005_lfm_train_0025_012_20251111T180648.mat");
% N_op=length(data.outputSignal);
cfgTxt = fileread('config_jitter_noreverb005_lfm_train_0025_012_20251111T180648.txt');
...
% 
% thr = load('page_thresholds_fixed_rnj_CNR30_pings100_NMC100_Pfa0.05.mat'); 
...
%     Res(m).tau0 = nan(1,Ns);
% 
%     % post-change delay stats (you already had these)
...
% 
%     % --- Build template that starts at ping 1 (proto)
%     cfgProto = cfgS;
...
%             Yrun = Yuse + E;
% 
%             % IMPORTANT: this must be your SINGLE-RUN function that returns .ell,.T,.det,...
...
% 
%         % --- Make a page trace for plotting from run #1 (guarantees fields exist)
%         out.page = struct();
...
% 
%         %%
%         det_all = out.mc.det_all;          % Nmc x Kuse
...
% 
%         % If first-ever detection is after tau0, it's already post-change
%         idx_direct = (~missed_global) & (tau_det_global >= tau0);
%         tau_det_post_change(idx_direct) = tau_det_global(idx_direct);
% 
%         % Only if first-ever detection is before tau0, we must look again after tau0
%         idx_need = (~missed_global) & (tau_det_global < tau0);
%         rows = find(idx_need).';
%         for rr = rows
%             idxPost = find(det_all(rr, tau0:Kuse) == uint8(2), 1, 'first');
%             if ~isempty(idxPost)
...
%         Res(m).out{isnr}=out;
% 
%         % --- Save one showcase run for Page trace plots
%         if isnr == isnr_show
...
restmp = load('Res_reverb_nojitter_movingtarget_block_CNR30_pfa0.05_ver2.mat');
Res    = restmp.Res;     % adjust if stored under a different name
% cfg    = restmp.cfg;     % if cfg was also saved in the .mat

%% ===================== PLOTS  =====================
SNRv = cfg.SNRdB_list(:).';   % SNR grid

%% (A) Page statistic trace at one SNR (run #1), one figure per model
%     (you can keep this block as-is if 'show' is present in Res)
for m = 1:numel(Res)
    ...
    xlabel('Ping index k');
    ylabel('T_k');
    % title(sprintf('%s: Page statistic (run #1), SNR = %g dB', ...
        % Res(m).name, sh.SNRdB));
end

%% (B) Mean detection delay vs SNR: E[ τ_det - τ0 ]
figure; hold on; grid on; box on;
for m = 1:numel(Res)
    delay_mean = Res(m).delay_mean; % mean delay = E[τ_det - τ0]

    plot(SNRv, delay_mean, 'LineWidth', 2);
end
xlabel('SNR [dB]');
ylabel('Mean delay (pings) to detection');
% title('Mean detection delay E[\tau_{det} - \tau_0] (censored)');
legend('$\mathcal{M}_0$','$\mathcal{M}_{cd}$','$\mathcal{M}_c$','$\mathcal{M}_d$','Interpreter','latex');

%% (C) Median detection delay vs SNR: median( τ_det - τ0 )
% figure; hold on; grid on; box on;
% for m = 1:numel(Res)
%     delay_med = Res(m).delay_med;   % median delay
% 
%     plot(SNRv, delay_med, 'LineWidth', 2);
% end
% xlabel('SNR [dB]');
% ylabel('Mean delay (pings) to detection');
% title('Median detection delay median(\tau_{det} - \tau_0) (censored)');
% legend('$\mathcal{M}_0$','$\mathcal{M}_{cd}$','$\mathcal{M}_c$','$\mathcal{M}_d$','Interpreter','latex');


%% (D1) P_d vs SNR (by end of horizon) – separate figure
figure; hold on; grid on; box on;
for m = 1:numel(Res)
...
xlabel('SNR [dB]');
ylabel('P_d');
% title('P_d by end of horizon (post-change)');
legend('$\mathcal{M}_0$','$\mathcal{M}_{cd}$','$\mathcal{M}_c$','$\mathcal{M}_d$','Interpreter','latex');


%% (D2) P_d vs SNR (within Kwin pings after τ0) – separate figure
% Kwin should be stored somewhere (often in the same .mat)
if isfield(restmp,'Kwin')
    Kwin = restmp.Kwin;
else
    % fallback if not saved; set manually:
    Kwin = 20;  % <-- adjust to your actual window
end
...
xlabel('SNR [dB]');
ylabel('P_d (windowed)');
% title(sprintf('P_d within %d pings after \\tau_0', Kwin));
legend('$\mathcal{M}_0$','$\mathcal{M}_{cd}$','$\mathcal{M}_c$','$\mathcal{M}_d$','Interpreter','latex');
